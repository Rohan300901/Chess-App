class AssetManagerChess{
  static String images = "assets/images";
  static String animations = "assets/animation";


  static String userIcon = "$images/user_icon.png";
  static String faceBookIcon = "$images/facebook_logo.png";
  static String googleIcon = "$images/google.jpg";
  static String stocFishIcon = "$images/stockfish_1.png";
  static String chessImage = "$images/Chess_image_1.jpg";
  static String logInAnimation = "$animations/login_animation.json";
  static String rookAnimation = "$animations/rook.json";
  static String knightAnimation = "$animations/knight_animation.json";
}